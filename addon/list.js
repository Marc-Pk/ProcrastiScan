...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (Array.isArray(distractingEntries)) {
      ...
      distractingEntries.forEach(entry => {
        ...
        link.style.color = '#3498db'; // Darkly-native primary color for links
    ...
    document.getElementById('deleteSelectedBtn').addEventListener('click', async () => {
      ...
      const updatedEntries = distractingEntries.filter((entry, index) => !selectedEntries.indexOf(index) != -1);
